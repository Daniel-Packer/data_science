...
def knight_features(game_dict):
    ...
    if (len(game_dict['white_pieces'][two_minor_pieces_turns(game_dict)[0]]['N']) == 2):
        knight_pair = 1
    ...
    if (not (game_dict['middle_game_index'] == None)):
        for turn in range(int(game_dict['middle_game_index']/2), int(end_game_index/2)):
            ...
            knight_outpost_turns += detect_outpost(game_dict, turn, player)
            ...
            for move in board.legal_moves:
                if (str(board.piece_at(move.from_square)) == 'N'):
                    knight_attack_counter += 1
    
        ## Scale the repositioning and attack counters by the total number of moves in the mid-game, so longer mid-games don't get overbiased.
