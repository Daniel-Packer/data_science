...
def bishop_features(game_dict):
    ...
    if (not (game_dict['middle_game_index'] == None)):
        ...
        long_diag_turns = 2 * long_diag_turns / abs(end_game_index - game_dict['middle_game_index']) ## The 2 * is so that a score of one would be all bishops on their long diagonals for the entire midgame, since we are dividing by the total number of half-turns, while the counted turns are only those of white
